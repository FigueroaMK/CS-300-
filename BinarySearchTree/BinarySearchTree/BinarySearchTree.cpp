...
struct Node {
    ...
    // Constructor initialized with a bid
    Node(Bid aBid) : Node() {
        this->bid = aBid;
    }
};
...
Bid BinarySearchTree::Search(string bidId) {
    ...
    while (current != nullptr) {  // keep looping downwards until bottom reached or matching bidId found
        if (current->bid.bidId.compare(bidId) == 0) {  // if match found, return current bid
            return current->bid;
        }
        else if (bidId.compare(current->bid.bidId) < 0) {   // if bid is smaller than current node then traverse left
...
void BinarySearchTree::addNode(Node* node, Bid bid) {
    ...
    if (node->bid.bidId.compare(bid.bidId) > 0) {  // if node is larger then add to left
        ...
        if (node->right == nullptr) {   // if no right node
            ...
            this->addNode(node->right, bid);   // recurse down the left node
        }
    }
}
void BinarySearchTree::inOrder(Node* node) {
    ...
    if (node != nullptr) {   //if node is not equal to null ptr
        ...
        cout << node->bid.bidId << " : "   //output bidID, title, amount, fund
            << node->bid.title << " | "
            << node->bid.amount << " | "
            << node->bid.fund << endl;
...
Node* BinarySearchTree::removeNode(Node* node, string bidId) {
    ...
    if (node == nullptr) {  // if node = nullptr return node
        return node;
    }
    if (bidId.compare(node->bid.bidId) < 0) {   // (otherwise recurse down the left subtree)
        node->left = removeNode(node->left, bidId);  // check for match and if so, remove left node using recursive call 
    }
    else if (bidId.compare(node->bid.bidId) > 0) {  // (otherwise recurse down the right subtree)
        ...
        if (node->left == nullptr && node->right == nullptr) {   // if left node = nullptr && right node = nullptr delete node 
            ...
            Node* temp = node; // if left node != nullptr && right node = nullptr delete node 
            node = node->left;
            delete temp;
        }
        else if (node->right != nullptr && node->left == nullptr) { // (otherwise check one child to the right)
            Node* temp = node; // if left node = nullptr && right node != nullptr delete node
            node = node->right;
            delete temp;
        }
        else {   // (otherwise more than one child so find the minimum)
            ...
            while (temp->left != nullptr) {   // while left node is not nullptr keep moving temp left
                temp = temp->left;// make node bid (right) equal to temp bid (left)
            }
            node->bid = temp->bid;   // remove right node using recursive call
            node->right = removeNode(node->right, temp->bid.bidId);// return node
        }
    }
    return node;
}
