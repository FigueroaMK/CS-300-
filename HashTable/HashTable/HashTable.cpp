...
// Description : Module 4 Hash Table
//============================================================================
...
private:
    ...
    struct Node {
        ...
        // initialized constructor with a bid
        Node(Bid aBid) : Node() {
            bid = aBid;
        }
...
void HashTable::Insert(Bid bid) {
    ...
    if (oldNode == nullptr) { // if no entry found for the key
        ...
        if (oldNode->key == UINT_MAX) {   //if the key is uint_max
            oldNode->key = key;           // assign the key
            oldNode->bid = bid;          //assign the bid
            oldNode->next = nullptr;    //assign the next node to the nullptr
        }
        else {    //otherwise chain the node until the next node is null
...
void HashTable::PrintAll() {
    ...
    for (unsigned int i = 0; i < tableSize; i++) {   // for node begin to end iterate
        Node* currNode = &nodes.at(i);
        
        if (currNode->key != UINT_MAX) {    //   if key not equal to UNIT_MAX
            
            cout << currNode->key << " : " << currNode->bid.bidId << " | " << currNode->bid.title << " | " << currNode->bid.amount
                << " | " << currNode->bid.fund << endl;   // output key, bidID, title, amount and fund
        }
        
        else {   // node is equal to next iter
            
            while (currNode->next != nullptr) {    // while node not equal to nullptr
                
                cout << currNode->key << " : " << currNode->bid.bidId << " | " << currNode->bid.title << " | " << currNode->bid.amount
                    << " | " << currNode->bid.fund << endl;   // output key, bidID, title, amount and fund
                
                currNode = currNode->next;    // node is equal to next node
            }
        }
    }
    return;
