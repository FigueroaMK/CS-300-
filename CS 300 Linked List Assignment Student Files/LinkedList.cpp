...
// Description : Module 3 Linked List
//============================================================================
...
LinkedList::LinkedList() {
    ...
    //set head and tail equal to nullptr (RESOLVED)
    head = nullptr;  //head set to nullptr
    tail = nullptr;  // tail set to nullptr
...
void LinkedList::Append(Bid bid) {
    ...
    Node* newNode = new Node(bid); //Creates a new node
    
    if (head == NULL) {    // if statment to determine and assign the head and tail nodes
        head = newNode;
        tail = newNode;
    }
    else {                 // else statement to assign a tail if a head is present
        tail->next = newNode;
        tail = newNode;
    }
    size++;         // increases the size count
}
...
void LinkedList::Prepend(Bid bid) {
    ...
    Node* newNode = new Node(bid); // Creates a new node
    if (head != NULL) {      //if statement to determine if head node is occupied
        newNode->next = head;
    }
    else {
         Append(bid);   // else call append
    }
    size++;    // increases the size count
...
void LinkedList::PrintList() {
    ...
    Node* currentNode = head;     //makes the current node the head node
    
    for (int i = 0; i < size; i++) {    //loop through the list

        cout << currentNode->bid.bidId << ":" << currentNode->bid.title << " , " << currentNode->bid.amount << " , " << currentNode->bid.fund << endl;
...
void LinkedList::Remove(string bidId) {
    ...
    for (int i = 0; i < size - 1; i++) {  //loop to identify the special cases
